...
class TogetherPipeline(object):
    ...
    def process_item(self, item, spider):
        if isinstance(item, UserItem):
            uid = item.get('uid')
            model = UserModel.select().where(UserModel.uid == uid).get()
            print(model)
            if model:
                UserModel.update({UserModel.nickname: item.get('nickname'),
                                  UserModel.head_pic: item.get('head_pic'),
                                  UserModel.update_time: datetime.now()}).where(UserModel.uid == uid).execute()
            else:
                UserModel.create(
                    uid=uid,
                    sex=item.get('sex'),
