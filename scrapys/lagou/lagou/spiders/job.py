...
class JobSpider(scrapy.Spider):
    ...
    def handle_location(self, response):
        ...
        if 0 == status:
            ...
            lat = location['lat']  # 纬度
            lng = location['lng']  # 经度
            diff_lat = item['company_latitude'] - lat
            diff_lng = item['company_longitude'] - lng
            if diff_lat > .1 and diff_lng > .1:
                if diff_lat > 1.0 or diff_lng > 1.0:
    ...
    def get_city():
        ...
        sql = 'select id,"name" from tb_city order by id asc limit 1'
        return sql
    ...
    def get_type():
        ...
        sql = 'select id,"name" from tb_type where id > 0 order by id asc limit 1'
        return sql
